...
interface Props {
  variant?: 'guides' | 'guides-inline' | 'era' | 'footer';
}
    ...
    return (
      <div className={`text-center ${variant === 'footer' ? 'py-3' : 'py-4'}`}>
        <p className="text-sm" style={{ color: '#c0a172' }}>
          ✓ You&apos;re in! Check your inbox for a free chapter from the Companion Guide.
        </p>
      </div>
    );
  }

  const isCompact = variant === 'era' || variant === 'footer' || variant === 'guides-inline';
    ...
    <div className={isCompact ? '' : 'mt-6 mb-8'}>
      {variant === 'guides' && (
        <div className="text-center mb-4">
          ...
          <p className="text-sm mt-1" style={{ color: '#8fa3b0' }}>
            Get a free chapter from the Companion Guide and see what is inside.
          </p>
        </div>
      )}
      {variant === 'era' && (
